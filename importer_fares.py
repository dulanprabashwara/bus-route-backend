"""
Official NTC Bus Fare Importer Pipeline
Supports:
...
# Verified stop aliases mapping (Sinhala name -> DB stop name_en)
VERIFIED_ALIASES = {
    "මහනුවර": "Kandy",
    "කොළඹ": "Colombo",
    "කෑගල්ල": "Kegalle",
...
def main():
    ...
    source_file_ids = {}
    files_to_register = [
        ("fare_sources/Inter Provincial Full Bus Fare.pdf", "Inter Provincial Full Bus Fare.pdf", "INTER_PROVINCIAL_ENDPOINT_FARES"),
        ("fare_sources/Semi Fares (Effect From 2026-07-06).pdf", "Semi Fares (Effect From 2026-07-06).pdf", "SEMI_LUXURY_ROUTE_MATRICES"),
        ("fare_sources/Normal Fares (Effect from 2026-07-06).pdf", "Normal Fares (Effect from 2026-07-06).pdf", "NORMAL_ROUTE_MATRICES")
    ]

    for rel_path, filename, f_type in files_to_register:
    ...
    cur.execute("SELECT id, name_en, normalized_name FROM stops WHERE active = TRUE")
    stops_rows = cur.fetchall()
    stop_name_to_id = {}
    for s_id, s_name_en, s_norm in stops_rows:
        if s_name_en:
            stop_name_to_id[s_name_en.strip().lower()] = s_id
        if s_norm:
            stop_name_to_id[s_norm.strip().lower()] = s_id

    print(f"Loaded {len(stops_rows)} active canonical stops into stop lookup dictionary.")
        ...
        for p_idx, page in enumerate(pdf.pages):
            ...
            for t in tables:
                for row in t:
                    ...
                    # Determine Origin / Destination stop IDs
                    # Match via VERIFIED_ALIASES or route origin/dest IDs
                    ...
                    if not from_stop_id or not to_stop_id or from_stop_id == to_stop_id:
                        # Fallback: query origin & destination from route_patterns if present
                    ...
                    if norm_fare_str and norm_fare_str != "--" and norm_fare_str != "-":
                        try:
                            ...
                            # Idempotent insert: DELETE existing match first, then INSERT
                            cur.execute("""
    ...
    print("Pass 1: Lightweight indexing of route pages...")
    route_page_index = {}
    ...
    # Find relevant pages for our 29 routable DB routes
    relevant_pages = []
    ...
    # Conflict check: compare Document C/D exact matrix endpoint fares vs Document E endpoint fares
    cur.execute("""
...
- **Idempotency**: Implemented via PostgreSQL `ON CONFLICT` unique constraints on `(route_pattern_id, from_stop_id, to_stop_id, service_type, fare_version_id)`. Re-running the importer updates amounts without creating duplicates.
