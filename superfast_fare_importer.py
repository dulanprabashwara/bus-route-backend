"""
Superfast Official NTC Bus Fare Importer Pipeline
Targeted Page Parsing Strategy:
1. Document E: Inter Provincial Full Bus Fare.pdf (FULL_ENDPOINT_ONLY)
2. Document D: Semi Fares (Effect From 2026-07-06).pdf (EXACT_POINT_TO_POINT, SEMI_LUXURY)
3. Document C: Normal Fares (Effect from 2026-07-06).pdf (EXACT_POINT_TO_POINT, NORMAL)
"""
...
def main():
    print("=" * 80)
    print("SUPERFAST OFFICIAL NTC BUS FARE IMPORTER PIPELINE")
    print("=" * 80)
    ...
    # SUPERFAST TARGETED PASS USING PDFMINER TEXT PRE-INDEXING
    # ═══════════════════════════════════════════════════════════════════
    print("\n" + "=" * 80)
    ...
    print("Step 1: Ultra-fast pypdf pre-indexing across 906 pages...")
    relevant_page_indices = []

    reader = pypdf.PdfReader(doc_c_path)
    for p_idx, page in enumerate(reader.pages):
        ...
        for l in text.split("\n"):
            ...
            if m:
                ...
                if r_num_found in target_route_numbers or clean_r_num in target_route_numbers:
                    relevant_page_indices.append((p_idx, r_num_found))
                break

    print(f"Pre-indexing complete! Found {len(relevant_page_indices)} target pages out of 906 pages.")

    doc_c_pages_indexed = 906
    doc_c_relevant_pages = len(relevant_page_indices)
    doc_c_matrices_parsed = 0
    doc_c_stops_matched = 0
    doc_c_unmatched_stops = []
    doc_c_fares_imported = 0

    print("Step 2: Parsing target tables with pdfplumber...")
    with pdfplumber.open(doc_c_path) as pdf:
        for p_idx, r_num_found in relevant_page_indices:
            clean_r_num = r_num_found.lstrip("0") or "0"
...
- **Strategy**: Superfast targeted pass using PDFminer text pre-indexing
