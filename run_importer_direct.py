"""
Direct, Ultra-Fast Official NTC Bus Fare Importer Pipeline
Supports:
...
def main():
    ...
    # ULTRA-FAST DIRECT TARGETED PASS
    # ═══════════════════════════════════════════════════════════════════
    print("\n" + "=" * 80)
    ...
    print(f"Scanning Document C (906 pages) for {len(target_route_numbers)} target routable DB routes...")
    ...
    with pdfplumber.open(doc_c_path) as pdf:
        for p_idx, page in enumerate(pdf.pages):
            ...
            if r_num_found not in target_route_numbers and clean_r_num not in target_route_numbers:
                continue
...
- **Strategy**: Ultra-fast direct targeted pass
